...
if [ "$(docker container ls --filter name=nginx -q)" ]; then
  ...
  if [ "$(docker container ls -a --filter name=demo_app_blue -q)" ]; then
    # Run the green app and stop the blue app
    docker-compose -f docker-compose.yml up -d --no-recreate
    docker container rm -f demo_app_blue
    echo "New version is demo app green"
  elif [ "$(docker container ls -a --filter name=demo_app_green -q)" ]; then
