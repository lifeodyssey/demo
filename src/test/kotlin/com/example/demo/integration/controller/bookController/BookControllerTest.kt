...
class BookControllerTest {
    ...
    @Test
    fun should_create_book_item_successfully() {//todo change book to books
        // Given
        val createBookRequest: MockHttpServletRequestBuilder = post("/book")
            .contentType(MediaType.APPLICATION_JSON)
            .content(bookJson.write(bookDto).json)
        whenever(bookService.createBook(bookDto)).thenReturn(bookID.toString())
        // When
    ...
    fun `findBookById should return book if found`() {
        ...
        whenever(bookService.findBookByID(bookID)).thenReturn(Optional.of(book))
        // When
